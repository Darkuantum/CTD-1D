...
`generateRandList(no_of_rand, lower_bound, upper_bound)` - Generates a list of no_of_rand unique random integers within the range `[lower_bound, upper_bound]` (exclusive). Ensures no duplicates.

`checkValid(str)` - Checks whether a given string contains only valid numeric characters, spaces, or periods. Converts any instances of commas to spacings. Ensures the string is not empty and provides feedback for invalid input.
